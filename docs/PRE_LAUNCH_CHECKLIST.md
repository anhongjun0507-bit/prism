...
## 4. 신규 로고 PNG/ICO 자산 제작

새 PrismLogo (단색 오렌지 삼각 + 빛줄기 3가닥)에 맞춰 다음 비-SVG 자산을 디자인 도구(Figma/Photoshop)에서 제작 후 교체:

- `src/app/favicon.ico` (Next.js 컨벤션, 16/32/48 multi-size)
- `public/icon-192.png`, `public/icon-512.png` (PWA manifest)
- `app/opengraph-image.tsx` 또는 og-image.png (소셜 미리보기)

SVG 자산(`public/icon.svg`, `public/favicon.svg`)은 이미 새 디자인으로 교체 완료.
